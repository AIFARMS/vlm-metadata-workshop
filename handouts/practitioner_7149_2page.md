<style>
@media print {
  .pagebreak { page-break-after: always; break-after: page; height: 0; margin: 0; border: 0; }
}
</style>

# Workshop handout — Image **7149**
### Comparing AI vision models (~2 pages · image 7149)

**Name:** _________________________ **Date:** _____________

---

## 1. Your observation first

*Look at the projected photo before reading model text.*

**Species:** _______________________________________________

**Scene / setting:** ________________________________________

**Time of day:** ____________________________________________

**Lighting:** _______________________________________________

**Weather:** ________________________________________________

**Background:** _____________________________________________

<div class="pagebreak"></div>

## 2. Three models — image 7149 (foggy night trail camera)

**Weather**
- GPT-4o: Fog or haze, misty / humid environment
- GPT-4.5: *(empty / not visible)*
- Llama: Foggy or misty

**Lighting**
- GPT-4o: Low light, consistent with nighttime
- GPT-4.5: Low-light, likely infrared or night-vision image
- Llama: Low light, likely nighttime or early morning

**Time**
- GPT-4o: Nighttime (timestamp and low light)
- GPT-4.5: Early morning (timestamp 3:30 AM)
- Llama: Early morning, around 3:30 AM

**Scene**
- GPT-4o: Open outdoor area, field, grass or vegetation
- GPT-4.5: Outdoor, natural grassy area
- Llama: Outdoor environment with fog or heavy mist

**Background**
- GPT-4o: Obscured by fog; faint grass/plant shapes lower frame
- GPT-4.5: Blurred, indistinct grass and open outdoor space
- Llama: Fog or heavy mist obscuring most details

*(Dataset: coyote trail-cam folder. CSV compares scene fields only — not species per model.)*

---

## 3. Do they agree? *(your judgment — not keyword matching)*

| Field | Y | Partial | N | What differs? |
|-------|---|---------|---|---------------|
| Weather | ☐ | ☐ | ☐ | |
| Lighting | ☐ | ☐ | ☐ | |
| Time | ☐ | ☐ | ☐ | |
| Scene | ☐ | ☐ | ☐ | |
| Background | ☐ | ☐ | ☐ | |

**Y** = same fact · **Partial** = overlap but someone adds/omits something · **N** = different claims

---

## 4. After instructor reveals scores

**Script verdict:** _________________________ *(hint: high divergence)*

**Biggest gap between your table and the script?** _________________________

**Auto-save to a database without review?** ☐ Yes ☐ No ☐ Some fields only

---

## 5. Discuss in pairs

1. When all three agree, is that enough to trust the label?
2. Where did models use **different words for the same idea**?
3. Where did they **disagree on facts**? *(Note: one model skipped weather.)*
4. One **classroom activity** using this compare-then-verify pattern:
   _________________________________________________________________
5. What images should **not** go to a cloud AI in your setting?

---

**Reference:** 334-image coyote set · 5 high-divergence outliers · **species not in comparison CSV** · agreement ≠ truth

*This image shows **real disagreement** on a hard foggy scene — not the same as 7225, where models mostly agree but use different words.*

*Optional depth: `technical_reference_1page.md` · Full worksheet: `PARTICIPANT_WORKSHEET.md`*
